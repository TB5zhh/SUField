...
class MinkUNetHyper(MinkUNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.final = nn.Sequential(conv(out_pool5 + out_pool6 + self.PLANES[6] + self.PLANES[0] * self.BLOCK.expansion, 512, kernel_size=1, bias=False, D=D),
                                   ME.MinkowskiBatchNorm(512), ME.MinkowskiReLU(), conv(512, out_channels, kernel_size=1, bias=True, D=D))
