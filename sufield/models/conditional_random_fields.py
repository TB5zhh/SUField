...
class MeanField(HighDimensionalModel):
    ...
    def __init__(self, nchannels, spatial_sigma, chromatic_sigma, meanfield_iterations, is_temporal, config, **kwargs):
        ...
        self.region_type_, self.region_offset_, _ = me_convert_region_type(conv.region_type, 1, conv.kernel_size, conv.up_stride, conv.dilation,
                                                                           conv.region_offset, conv.axis_types, conv.dimension)
    ...
    def initialize_coords(self, model, in_coords, in_color):
        if torch.prod(convert_to_int_tensor(model.OUT_PIXEL_DIST, model.D)) != 1:
            ...
            self.in_mapping = torch.sparse.FloatTensor(torch.stack((in_cols.long(), torch.arange(in_cols.size(0), out=torch.LongTensor()))),
                                                       torch.ones(in_cols.size(0)), torch.Size((self.n_rows, in_cols.size(0))))

            out_cols = self.get_index_map(orig_tri_coords, 1)
            self.out_mapping = torch.sparse.FloatTensor(torch.stack((torch.arange(out_cols.size(0), out=torch.LongTensor()), out_cols.long())),
    ...
    def forward(self, x):
        ...
        for i in range(self.meanfield_iterations):  # Meanfield iteration
            ...
            # Pairwise potential
            out = self.convs[i].apply(out, self.conv.kernel, x.pixel_dist, self.conv.stride, self.conv.kernel_size, self.conv.dilation, self.region_type_,
...
class BilateralCRF(Wrapper):
    ...
    def initialize_filter(self, NetClass, in_nchannel, out_nchannel, config):
        ...
        self.filter = MeanField(out_nchannel,
