...
class RandomDropout(object):
    ...
    def __init__(self, dropout_ratio=0.5, apply_ratio=0.2):
        ...
        self.dropout_application_ratio = apply_ratio
...
class RandomHorizontalFlip(object):
    ...
    def __init__(self, upright_axis, apply_ratio=[0.5, 0.5]):
        """
        upright_axis: axis index among x,y,z, i.e. 2 for z
        """
        self.D = 3
    ...
    def __call__(self, coords, feats, labels):
        for i, curr_ax in enumerate(self.horz_axes):
            if random.random() < self.apply_ratio[i]:
...
"""
Written by Beiwen Tian
"""

class Voxelize():

    def __init__(self, fix_map=None, voxel_size=0.05, return_map=False) -> None:
    ...
    def __call__(self, coords, feats, labels):
        ...
        if self.return_map:
            return np.asarray(voxelized_coords), np.asarray(voxelized_feats), np.asarray(voxelized_labels), np.asarray(indices_map), np.asarray(reverse_indices_map),
        else:
            return np.asarray(voxelized_coords), np.asarray(voxelized_feats), np.asarray(voxelized_labels)
...
class RandomRotation(object):
    ...
    def rotate_mat(self, axis, theta):
        """
    ...
    def __call__(self, coords, feats, labels):
        ...
        for axis_index, boundary in enumerate(self.boundaries):
            ...
            rotation_mat = rotation_mat @ self.rotate_mat(axis, theta)
        rotated_coords = coords @ rotation_mat.T
        return rotated_coords, feats, labels
...
class NonNegativeTranslation(object):
    def __call__(self, coords, feats, labels):
        min_coord = coords.min(0)
        translated_coords = coords - min_coord
        return translated_coords, feats, labels
