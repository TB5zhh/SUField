...
def print_info(iteration, max_iteration, data_time, iter_time, has_gt=False, losses=None, scores=None, ious=None, hist=None, ap_class=None, class_names=None):
...
def test(model, data_loader, config, transform_data_fn=None, has_gt=True):
        ...
        for iteration in range(max_iter):
            ...
            if has_gt:
                if config.evaluate_original_pointcloud:
                    ...
                    output, pred, target = permute_pointcloud(coords, pointcloud, transformation, dataset.label_map, output, pred)
    ...
    reordered_ap_class = dataset.reorder_result(ap_class)
    class_names = dataset.get_classnames()
    print_info(iteration, max_iter_unique, data_time, iter_time, has_gt, losses, scores, reordered_ious, hist, reordered_ap_class, class_names=class_names)
